...
def collapse_almost_empty_rows(
    ...
    for _, row in data_frame.iterrows():
        if _is_almost_empty_row(row):
            ...
            for previous_value, new_value in zip(
                previous_row, row.values, strict=True
            ):
                if new_value != "":
                    if isinstance(previous_value,
                                  str) and isinstance(new_value, str):
                        value = f"{previous_value} {new_value}"
                    else:
                        logger.warning(
                            "Found almost empty row with non-string data, using previous: %s + %s",

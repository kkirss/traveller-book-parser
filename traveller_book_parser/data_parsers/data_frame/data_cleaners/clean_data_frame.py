...
def _log_cleaned_data_frame(data_frame: DataFrame, clean_type: str) -> None:
...
def clean_data_frame(input_data_frame: DataFrame) -> DataFrame:
    ...
    while True:
        if (new_data_frame := remove_unnamed_columns(data_frame)) is not None:
            ...
            _log_cleaned_data_frame(data_frame, remove_unnamed_columns.__name__)
            continue

        if (new_data_frame := collapse_almost_empty_rows(data_frame)) is not None:
            data_frame = new_data_frame
            _log_cleaned_data_frame(data_frame, collapse_almost_empty_rows.__name__)
            continue

        if (new_data_frame := spread_returns_to_multiple_rows(data_frame)) is not None:
            data_frame = new_data_frame
            _log_cleaned_data_frame(
                data_frame, spread_returns_to_multiple_rows.__name__
            )
            continue
        ...
        if (new_data_frame := replace_few_returns_with_spaces(data_frame)) is not None:
            data_frame = new_data_frame
            _log_cleaned_data_frame(
                data_frame, replace_few_returns_with_spaces.__name__
            )
            continue
